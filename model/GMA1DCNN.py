...
class GMA_1DCNN(nn.Module):
    ...
    def extract_features(self, x):
        ...
        x_transformer_in = x.transpose(1, 2)
        features['transformer_input'] = x_transformer_in

        x = self.attention_block(x_transformer_in)
        features['transformer_output'] = x


        x_flat = x.reshape(x.size(0), -1)
        features['flatten'] = x_flat


        fc_input = self.fc[0](x_flat)
        features['fc_input'] = fc_input
