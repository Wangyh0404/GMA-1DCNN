...
# ====================== 基础读取与变换 ======================
def read_first_col_xlsx(path: str) -> np.ndarray:
    s = pd.read_excel(path, usecols=[0], header=None, engine="openpyxl").squeeze("columns")
    return s.dropna().astype(np.float32).values

def rfft_half_drop_nyquist(wins_2d: np.ndarray) -> np.ndarray:
    spec = np.fft.rfft(wins_2d, axis=1)           # (N, W/2+1)
    mag  = (np.abs(spec) / wins_2d.shape[1])      # 幅值按长度缩放（非标准化）
    return mag[:, :-1].astype(np.float32)         # 丢奈奎斯特点 → (N, W/2)
...
def add_awgn_per_window(wins_2d: np.ndarray, snr_db: float, rng: np.random.RandomState) -> np.ndarray:
    ...
    noise_pow = np.mean(noise**2, axis=1, keepdims=True) + 1e-12
    noise = noise * np.sqrt(noise_var / noise_pow)
...
def load_mfs_sequential_no_cross_overlap(
    datadir: str,
    load_type: str,
    label_map: Dict[str, int],
    window_size: int = 1024,
    overlap: float = 0.5,
    # 每文件 (train/val/test) 采样窗口个数
    ...
    for fname in sorted(os.listdir(base)):
        ...
        if label_name not in label_map:
            print(f"[WARN] skip unknown label file: {fname}")
            continue
        lab = label_map[label_name]

        arr = read_first_col_xlsx(str(base / fname))
        ...
        print(f"[OK] {load_type}/{fname} -> train={Xtr.shape[0]} val={Xva.shape[0]} test={Xte.shape[0]}")
    ...
    print(f"[SHAPE] TRAIN: {Xtr.shape}, VAL: {Xva.shape}, TEST: {Xte.shape} | "
